...
| L3 页面 | `{主题域}.md` | `AI技术/深度学习.md`（在 0101-0104 子目录下）|
...
├── 0102-wiki-concepts/   # L3 概念加工
├── 0103-wiki-entities/   # L3 实体档案
├── 0104-wiki-comparisons/# L3 对比分析
...
7. 判断是否需要新建/更新 L3 加工页面，单页条目上限 50 条
8. 追加 `0109-log/LOG-YYYY-MM-DD.md`
9. 报告结果并询问是否删除 inbox 原文件（默认删除）
10. git commit

### Query（L3 → L2 → L1）

1. 优先检索 L3 `0101-0104/`（title/processing_path/summary/正文）
2. L3 不足时回查 L2 主题目录
3. L2 仍不足且 L1 仍保留时，最后回查 `0003-inbox/`
4. 默认读 Top 8 全文；不足时扩大范围并在回答中说明检索层级
5. 回答必须引用 `[[wikilink]]` 或文件路径
...
- 所有路径使用正斜杠 `/`
- wikilink 推荐使用页面标题 `[[transformer]]`，不推荐使用路径 `[[3001-深度学习/transformer|标题]]`
