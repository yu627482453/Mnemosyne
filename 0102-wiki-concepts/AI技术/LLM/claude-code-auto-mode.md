...
它在输入侧对工具返回内容做 prompt injection 探测，在输出侧对 agent transcript 做分级判定。系统将不同动作分层处理：安全白名单工具、项目内文件操作，以及需要 classifier 介入的更高风险动作。
...
<!-- planned_links: [[prompt-injection]] [[agent-safety]] [[permission-model]] -->
